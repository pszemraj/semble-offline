...
def _verify_metadata(path: Path, wheel: zipfile.ZipFile, names: set[str], manifest: dict[str, Any]) -> None:
    ...
    wheel_metadata_path = next((name for name in names if re.fullmatch(r"[^/]+\.dist-info/WHEEL", name)), None)
    package_metadata_path = next((name for name in names if re.fullmatch(r"[^/]+\.dist-info/METADATA", name)), None)
    if wheel_metadata_path is None or package_metadata_path is None:
...
def _platform_details(manifest: dict[str, Any]) -> tuple[str, str]:
    ...
    if suffix not in {".so", ".dylib"}:
        raise RuntimeError(f"Unsupported grammar library suffix: {suffix!r}")
    if (operating_system, platform_data.get("architecture"), suffix) not in {
        ("linux", "x86_64", ".so"),
        ("macos", "arm64", ".dylib"),
    }:
        raise RuntimeError("Manifest platform, architecture, and library suffix are inconsistent")
    return operating_system, suffix
